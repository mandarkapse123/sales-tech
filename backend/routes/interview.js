...
function syncEntryToSearch(entry, tabName) {
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?)
  `).run(`interview:${tabName}`, entry.id, entry.title, entry.content || '', tagsStr);
}

/* ── INTERVIEW TABS ── */

// GET all interview tabs
router.get('/tabs', (req, res) => {
  try {
    const tabs = db.prepare('SELECT * FROM interview_tabs ORDER BY sort_order ASC').all();
...
// POST create custom interview tab
router.post('/tabs', (req, res) => {
  try {
    const { name, icon } = req.body;
    if (!name) return res.status(400).json({ success: false, error: 'name required' });
    const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
    const existing = db.prepare('SELECT id FROM interview_tabs WHERE slug = ?').get(slug);
    if (existing) return res.status(400).json({ success: false, error: 'Tab with this name already exists' });
    const id = uuidv4();
    const maxOrder = db.prepare('SELECT MAX(sort_order) as m FROM interview_tabs').get();
    db.prepare(`
      INSERT INTO interview_tabs (id, name, slug, icon, is_builtin, sort_order) VALUES (?, ?, ?, ?, 0, ?)
    `).run(id, name, slug, icon || '📌', (maxOrder.m || 0) + 1);
    const tab = db.prepare('SELECT * FROM interview_tabs WHERE id = ?').get(id);
...
// DELETE interview tab (non-builtin only)
router.delete('/tabs/:id', (req, res) => {
  try {
    const tab = db.prepare('SELECT * FROM interview_tabs WHERE id = ?').get(req.params.id);
    ...
    db.prepare('DELETE FROM interview_tabs WHERE id = ?').run(req.params.id);
...
router.get('/entries/:tabId', (req, res) => {
  try {
    ...
    const entries = db.prepare(`
      SELECT * FROM interview_entries WHERE tab_id = ? ORDER BY is_pinned DESC, created_at DESC
    `).all(tabId);
...
router.post('/entries/:tabId', (req, res) => {
  try {
    ...
    const tab = db.prepare('SELECT * FROM interview_tabs WHERE id = ?').get(tabId);
    if (!tab) return res.status(404).json({ success: false, error: 'Tab not found' });
    const id = uuidv4();
    db.prepare(`
      INSERT INTO interview_entries (id, tab_id, title, content, type, file_path, original_filename, tags, is_pinned)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, tabId, title, content || '', type || 'text', file_path || null, original_filename || null, JSON.stringify(tags || []), is_pinned ? 1 : 0);
    const entry = db.prepare('SELECT * FROM interview_entries WHERE id = ?').get(id);
...
// PUT update interview entry
router.put('/entries/:id', (req, res) => {
  try {
    const { title, content, type, file_path, original_filename, tags, is_pinned } = req.body;
    const existing = db.prepare('SELECT * FROM interview_entries WHERE id = ?').get(req.params.id);
    if (!existing) return res.status(404).json({ success: false, error: 'Not found' });
    db.prepare(`
      UPDATE interview_entries SET title=?, content=?, type=?, file_path=?, original_filename=?, tags=?, is_pinned=?, updated_at=CURRENT_TIMESTAMP WHERE id=?
    `).run(
    ...
    const entry = db.prepare('SELECT * FROM interview_entries WHERE id = ?').get(req.params.id);
    const tab = db.prepare('SELECT * FROM interview_tabs WHERE id = ?').get(entry.tab_id);
    entry.tags = JSON.parse(entry.tags);
    entry.is_pinned = Boolean(entry.is_pinned);
...
router.delete('/entries/:id', (req, res) => {
  try {
    const row = db.prepare('SELECT * FROM interview_entries WHERE id = ?').get(req.params.id);
